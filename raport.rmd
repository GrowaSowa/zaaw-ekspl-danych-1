...
## Podsumowanie
Badania są w większości skoncentrowane na elektrodach z elektrolitem wykonanym z wodorotlenku potasu (KOH) lub kwasu siarkowego (H~2~SO~4~). Najczęściej występujące materiały posiadają w badaniach lepsze rezultaty, choć znaczna większość przeprowadzonych badań raportuje względnie niską pojemność badanych elektrod (< 500 F/g).
...
W zestwieniu pominięto atrybut konfiguracji elektrody, gdyż jest to atrybut tekstowy z aż `r length(unique(data$Electrode.Configuration))` różnymi wartościami. Nie jest możliwe zaprezentowanie rozkładu wartości tego atrybutu w raporcie w czytelny sposób.
...
# fld_lst <- list(
#   list(method = "update",
#        args = list(list(y = list(data_filtered$Current.Density..A.g.)),
#                    list(yaxis = list(title = "Current.Density..A.g."))),
#        label = "Current.Density..A.g."),
#   list(method = "update",
#        args = list(list(y = list(data_filtered$Electrolyte.Ionic.Conductivity)),
#                    list(yaxis = list(title = "Electrolyte.Ionic.Conductivity"))),
#        label = "Electrolyte.Ionic.Conductivity"),
#   list(method = "update",
#        args = list(list(y = list(data_filtered$Electrolyte.Concentration..M.)),
#                    list(yaxis = list(title = "Electrolyte.Concentration..M."))),
#        label = "Electrolyte.Concentration..M.")
# )
...
# fig <- fig %>%
#   layout(updatemenus = list(list(active = 1, x = -0.05, y = 1.1,
#                                  xanchor = "center", buttons = fld_lst)))
...
mse <- mean((testing_data$Capacitance..F.g. - predictions)^2)
```

Błąd średniokwadratowy modelu wynosi `r format(mse, scientific = FALSE)`.
